  ...
  return (
    <Grid  mt={4} mb={10} container gap={4} spacing={2} justifyContent='center' alignItems='center'>
      <Grid item xs={10} md={10}>
        <Typography color={ColorsKlinik.text} fontSize={18}>Aqui puedes añadir algún dia en especifico en el cual no estaras disponible(recuerda que tiene que ser 5 días antes)</Typography>
      </Grid>
      <Grid item md={4}>
      ...
      <Grid id='cardsContainer' item md={6}>
        {values.map((value) => (
                  <Card sx={{margin: 5}}>
                  <CardHeader
                    titleTypographyProps={{variant:'h7' , color: ColorsKlinik.text } }
                    title={value.title}
                    action={
